  ...
  if (!isAdmin) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center rounded-[2rem] border border-dashed border-red-200 bg-red-50 p-8 text-center text-red-600">
        ...
        <h2 className="text-xl font-extrabold">غير مصرح لك بدخول الخزنة</h2>
        <p className="mt-2 max-w-md text-sm font-bold leading-6">
          الصفحة محمية للمدير فقط.
        </p>
      </div>
    );
  }
    ...
    <div className="mx-auto flex h-full max-w-6xl flex-col gap-6">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#0A2540]/40">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-bold text-slate-500 dark:text-slate-400">الوراد</p>
          </div>
      ...
      >
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-slate-500 dark:text-slate-400">
              لا توجد بيانات وهمية. أول حركة حقيقية هتظهر هنا بعد الربط.
            </p>
          </div>
        </div>
        ...
        <EmptyState
          ...
          description="الصفحة الحالية عبارة عن لوحة متابعة نظيفة بدون أي سجل وهمي. بعد Supabase هنقرا الحركات الفعلية ونقسمها تلقائياً."
        />
      </motion.section>
    </div>
  );
